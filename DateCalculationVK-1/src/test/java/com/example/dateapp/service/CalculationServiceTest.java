...
public class CalculationServiceTest {
	...
	public static class 日付計算例外処理 {
		...
		private DateFormula setUpFormula(int 加減年, int 加減月, int 加減日) {
			DateFormula foumula = new DateFormula();
			foumula.setAdjustmentYear(加減年);
			foumula.setAdjustmentMonth(加減月);
			foumula.setAdjustmentDay(加減日);
			return foumula;
		}

		@Test
		public void 計算基準日にNULLを渡すとNullPointerExceptionとなること() throws Exception {
			assertThatThrownBy(() -> {
		...
		@Test
		public void 計算機準備をyyyyMMdd以外の形式で渡すとDateTimeParseExceptionとなること() throws Exception {
			assertThatThrownBy(() -> {
		...
		@Test
		public void 日付計算式にNULLを渡すとNullPointerExceptionとなること() throws Exception {
			assertThatThrownBy(() -> {
