...
public class UpdateControllerTest {
	...
	@Test
	public void 更新ページで日付名が20桁以上の状態で更新処理を行うと例外情報が入った状態で画面が返る事() throws Exception {
		sut.perform(post("/update/{dateId}", "Y01").param("dateId", "Y01").param("dateName", "010101010101010101010"))
				.andExpect(status().isOk())
				.andExpect(view().name("update"));
	}
